...
두 큐에 담긴 모든 원소의 합은 20입니다. 따라서, 각 큐의 합을 10으로 만들어야 합니다. queue2에서 1,
10을 순서대로 추출하여 queue1에 추가하고, queue1에서 1, 2, 1, 2와 1(queue2으로부터 받은 원소)을 순서대로 추출하여 queue2에 추가합니다.
그 결과 queue1은 [10], queue2는 [1, 2, 1, 2, 1, 2, 1]가 되며, 각 큐의 원소 합은 10으로 같습니다. 이때 작업 횟수는 7회이며,
이보다 적은 횟수로 목표를 달성하는 방법은 없습니다. 따라서 7를 return 합니다.
...
def solution(queue1, queue2):
    ...
    if sum_que1 == sum_que2:
        return 0
    for i in range(1, (len_que1 + len_que2) * 2):
        pop_ = 0

        if sum_que1 > sum_que2:
            pop_ = so1_que1.popleft()
            so1_que2.append(pop_)
            sum_que1 -= pop_
            sum_que2 += pop_
        elif sum_que2 > sum_que1:
            pop_ = so1_que2.popleft()
            so1_que1.append(pop_)
            sum_que1 += pop_
            sum_que2 -= pop_
