...
def solution(priorities, location):
    answer = 0
    pri = [pri for pri in enumerate(priorities)]
    sort_pri = deque(sorted(priorities, reverse=True))
    temp = deque(pri)
    while temp:
        pop_temp = temp.popleft()

        if pop_temp[1] != sort_pri[0]:
            temp.append(pop_temp)
        else:
            answer += 1
            if pop_temp[0] == location:
                break
            sort_pri.popleft()
            print(pop_temp)
...
"""
any() -> 리스트 내부에 하나라도 True인게 있으면 True
all() -> 리스트 내부가 모두 True여야 True 반환

any를 사용했다면 불필요한 정렬과정을 거치지 않았어도 됐다. 
좀 더 효율적으로 짤 수 있게 되보자
"""